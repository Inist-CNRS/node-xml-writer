...
XMLWriter.prototype = {
    ...
    startElement : function (name) {
        name = strval(name);
        if (!name.match(this.name_regex)) throw Error('Invalid Parameter: name');
        if (this.tags === 0 && this.root && this.root !== name) throw Error('Invalid Parameter: name');
        if (this.attributes) this.endAttributes();
        ++this.tags;
        this.texts = 0;
    ...
    startElementNS : function (prefix, name, uri) {
        ...
        if (!prefix.match(this.name_regex)) throw Error('Invalid Parameter: prefix');
        if (!name.match(this.name_regex)) throw Error('Invalid Parameter: name');
        if (this.attributes) this.endAttributes();
        ++this.tags;
        this.texts = 0;
    ...
    startDocType : function (name, pubid, sysid, subset) {
        ...
        if (!name.match(this.name_regex)) throw Error('Invalid Parameter: name');
        if (pubid && !pubid.match(/^[\w\-][\w\s\-\/\+\:\.]*/)) throw Error('Invalid Parameter: pubid');
        if (sysid && !sysid.match(/^[\w\.][\w\-\/\\\:\.]*/)) throw Error('Invalid Parameter: sysid');
        if (subset && !subset.match(/[\w\s\<\>\+\.\!\#\-\?\*\,\(\)\|]*/)) throw Error('Invalid Parameter: subset');
